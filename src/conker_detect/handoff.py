...
def prepare_ledger_handoff(
    ...
    *,
    ...
    bundle_id = bundle_id or run_dir.name
...
def _synthesize_audits(
    ...
    tier1 = _combine_status(
        [
            submission_report.get("verdict"),
            provenance_report.get("verdict") if provenance_report is not None else None,
        ]
    )
    out: dict[str, Any] = {
        "tier1": {
            "status": tier1,
            "submission": submission_report.get("verdict"),
            "provenance": provenance_report.get("verdict") if provenance_report is not None else None,
        }
    }
