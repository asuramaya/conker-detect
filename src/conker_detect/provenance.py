...
def audit_provenance(source: Path | str | dict[str, Any]) -> dict[str, Any]:
    ...
    checks_and_rows = {
        "selection_disclosure": check_selection_disclosure(manifest),
        "dataset_fingerprints": check_dataset_fingerprints(manifest),
        "held_out_identity": check_held_out_identity(manifest),
    }
...
def check_selection_disclosure(manifest: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    selection = manifest.get("selection", {})
    findings: list[dict[str, Any]] = []
    if not selection:
    ...
    if selection_mode == "best_of_k":
        if candidate_run_count is None:
            findings.append(
                finding(
                    "warn",
                    "missing_candidate_run_count",
                    "best_of_k selection was declared without candidate_run_count",
                )
            )
            ...
            findings.append(
                finding(
                    "warn",
                    "inconsistent_selection_population",
                    "best_of_k selection was declared with candidate_run_count <= 1",
                    details={"candidate_run_count": int(candidate_run_count)},
                )
            )
        else:
            findings.append(
                finding(
                    "warn",
                    "selection_bias_risk",
                    "best_of_k selection was disclosed; reported metrics may be optimistic without an unseen test set",
                    details={"candidate_run_count": int(candidate_run_count)},
                )
            )
        ...
        findings.append(
            finding(
                "warn",
                "selection_population_without_mode",
                "candidate_run_count > 1 was declared without best_of_k selection_mode",
                details={"candidate_run_count": int(candidate_run_count), "selection_mode": selection_mode},
            )
        )
...
def check_dataset_fingerprints(manifest: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    datasets = manifest.get("datasets", {})
    findings: list[dict[str, Any]] = []
    if not datasets:
    ...
    for name, entry in normalized.items():
        if entry["fingerprint"] is None:
            findings.append(
                finding("warn", "missing_dataset_fingerprint", f"Dataset {name} is missing a fingerprint")
            )
    ...
    for lhs, rhs in (("train", "validation"), ("train", "held_out_test"), ("validation", "held_out_test")):
        ...
        if lhs_fp is not None and rhs_fp is not None and lhs_fp == rhs_fp:
            findings.append(
                finding(
                    "fail",
                    "dataset_fingerprint_overlap",
                    f"Datasets {lhs} and {rhs} have the same fingerprint",
                    details={"lhs": lhs, "rhs": rhs, "fingerprint": lhs_fp},
                )
            )
...
def check_held_out_identity(manifest: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    datasets = manifest.get("datasets", {})
    findings: list[dict[str, Any]] = []
    ...
    if held_out["name"] is None:
        findings.append(
            finding(
                "warn",
                "missing_held_out_identity",
                "Held-out test identity was not declared in the provenance manifest",
            )
        )
    if held_out["name"] is not None and held_out["fingerprint"] is None:
        findings.append(
            finding(
                "warn",
                "missing_held_out_fingerprint",
                "Held-out test dataset was named but is missing a fingerprint",
                details={"name": held_out["name"]},
            )
        )
    return {
        "pass": not any(row["severity"] == "fail" for row in findings),
