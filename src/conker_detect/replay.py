...
def _merge_batch_repeat_stats(
    ...
    *,
    ...
    local_max = float(np.max(pred_diff, initial=0.0))
    target["max_abs_prediction_diff"] = max(float(target["max_abs_prediction_diff"]), local_max)
    ...
    local_gold_max = float(np.max(gold_diff, initial=0.0))
    target["max_abs_gold_logprob_diff"] = max(float(target["max_abs_gold_logprob_diff"]), local_gold_max)
...
def _merge_repeat_stats(target: dict[str, Any], source: dict[str, Any]) -> None:
    for key in (
        ...
        "position_count",
        "prediction_diff_sum",
        "prediction_diff_count",
        "prediction_diff_failures",
        "gold_logprob_diff_sum",
        "gold_logprob_diff_count",
        "gold_logprob_diff_failures",
        "state_hash_mismatch_count",
        "state_hash_compared_count",
    ):
        target[key] += source[key]
    target["max_abs_prediction_diff"] = max(
        float(target["max_abs_prediction_diff"]),
        float(source["max_abs_prediction_diff"]),
    )
    target["max_abs_gold_logprob_diff"] = max(
        float(target["max_abs_gold_logprob_diff"]),
        float(source["max_abs_gold_logprob_diff"]),
    )
...
def _finalize_repeat_stats(stats: dict[str, Any], *, atol: float, rtol: float) -> dict[str, Any]:
    ...
    mean_abs_prediction_diff = (
        float(stats["prediction_diff_sum"] / stats["prediction_diff_count"])
        if stats["prediction_diff_count"]
        else None
    )
    mean_abs_gold_logprob_diff = (
        float(stats["gold_logprob_diff_sum"] / stats["gold_logprob_diff_count"])
        if stats["gold_logprob_diff_count"]
        else None
    )
    pass_value = None
    if compared:
        pass_value = (
            int(stats["prediction_diff_failures"]) == 0
            and int(stats["gold_logprob_diff_failures"]) == 0
            and int(stats["state_hash_mismatch_count"]) == 0
        )
    return {
        "covered": compared,
        "pass": pass_value,
